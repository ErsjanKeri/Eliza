 ...
 *
 * This registry implements the MatFormer concept where E2B is a nested subset of E4B, allowing
 * for efficient variant switching without requiring separate model downloads.
 *
 * Migrated from enum-based to String-based variant approach while preserving all sophisticated
 * device capability analysis and performance optimization features.
 */
@Singleton
class ElizaModelRegistry @Inject constructor(@ApplicationContext private val context: Context) {
    ...
    private fun initializeDefaultModels() {
                ...
                mapOf(
                    ...
                    "E2B" to
                        VariantConfig(
                            ...
                            3_500_000_000L, // ~3.5GB with PLE optimization
        ...
        Log.d(
            TAG,
            "Initialized ModelRegistry with Gemma model containing ${gemmaModel?.availableVariants?.size} variants"
        )
    }
    ...
    /**
     * Switches to a specified variant. This is a high-level operation that coordinates the variant
     * change.
     */
    ...
    /**
     * Get recommended variant based on device capabilities. This implements intelligent variant
     * selection using MatFormer principles.
     */
    ...
    /**
     * Get recommended variant based on current device state. This is a convenience overload that
    ...
    fun getAvailableVariantsWithPerformance(): List<ModelPerformance> {
        return getAvailableVariants().mapNotNull { variant -> getVariantPerformance(variant) }
    }
    ...
    fun getOptimalVariant(
        ...
        return availableVariants
            ...
            .filter { (variant, performance) ->
                ...
                maxMemoryGB?.let { maxMem -> perf.estimatedMemoryMB / 1024 <= maxMem } ?: true
            }
            .filter { (variant, performance) ->
            ...
            ?.first
            ?: getRecommendedVariant(deviceCapabilities)
    }
    ...
    fun getVariantRecommendationWithReason(): VariantRecommendation {
        ...
        val reason =
            when {
                deviceCapabilities.isLowMemory ->
                    "Device is low on memory, using memory-optimized variant"
                deviceCapabilities.availableMemoryGB < 6 ->
                    "Limited memory available (${deviceCapabilities.availableMemoryGB}GB), using efficient variant"
                deviceCapabilities.preferPerformance ->
                    "High-performance device detected, but recommending balanced variant for stability"
